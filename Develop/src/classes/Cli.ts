  ...
  findVehicleToTow(truck: Truck): void {
    inquirer
      ...
      .then((answers) => {
        ...
        if(!vehicleToTow) {
          console.error("No vehicle to tow!")
        }
            ...
            if (this.vehicles[i].vin === this.selectedVehicleVin){
              ...
              this.findVehicleToTow(this.vehicles[i] as Truck);
               return;
                ...
                console.log(`This vehicle cannot Tow. Please select a Truck to begin towing.`);
                this.performActions();
                return;
              }
            }
        ...
        // DONE: add statements to perform the wheelie action only if the selected vehicle is a motorbike
          ...
          for (let i=0; i< this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin) {
              ...
              console.log(`This vehicle cannot do a Wheelie. Please select a Motorbike.`)
              this.performActions();
              return;
            }
            }
